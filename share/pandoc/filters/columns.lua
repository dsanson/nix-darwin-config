...
--- Formats columns for ConTeXt output.
-- @param elem Pandoc element (Div of "columns" class) containing the
--    columns to be formatted.
-- @return elem with suitable RawBlocks in ConTeXt added
local function format_columns_context(elem)

  -- how many columns?
  number_columns = determine_column_count(elem)
  local column_opts = 'n=' .. number_columns
  local context_command = 'columns'

  if elem.attr.attributes["column-rule"] or
     elem.attr.classes:includes("rule") then
    column_opts = column_opts .. ',separator=rule'
  end
  if elem.attr.classes:includes("boxed") then
    column_opts = column_opts .. ',frame=on'
  end

  local context_begin = '\\start' .. context_command .. '[' .. column_opts ..']'
  local context_end = '\\stop' .. context_command

  -- wrap elem in raw conTeXt
  elem.content:insert(1, pandoc.RawBlock('context', context_begin))
  elem.content:insert(pandoc.RawBlock('context', context_end))
  ...
  filter = {
    ...
    Div = function(el)
      ...
      if el.classes:includes("columnbreak") then
        return pandoc.RawBlock('context', "\\columnbreak\n")
      end
  ...
  Div = function (element)
    ...
    if element.classes:includes ("columns") then
      ...
      elseif FORMAT:match('context') then
        element = format_columns_context(element)
